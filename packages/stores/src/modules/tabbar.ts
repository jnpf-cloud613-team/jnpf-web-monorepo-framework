...
import { openRouteInNewWindow, startProgress, stopProgress } from '@vben-core/shared/utils';
...
export const useTabbarStore = defineStore('core-tabbar', {
  actions: {
    ...
    async updateCacheTabs() {
      if (!keepAlive.value) return (this.cachedTabs = [] as any);
  ...
  state: (): TabbarState => ({
    ...
    menuList: ['close', 'affix', 'reload', 'close-left', 'close-right', 'close-other', 'close-all', 'collect'],
