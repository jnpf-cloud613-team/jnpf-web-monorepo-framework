...
export function findNode<T = any>(tree: any, func: Fn, config: Partial<TreeHelperConfig> = {}): null | T {
  config = getConfig(config);
...
export function findPath<T = any>(tree: any, func: Fn, config: Partial<TreeHelperConfig> = {}): null | T | T[] {
  config = getConfig(config);
  const path: T[] = [];
  const list = [...tree];
 ...
 * @description: 提取树指定结构
 */
export function treeMapEach(data: any, { conversion, children = 'children' }: { children?: string; conversion: Fn }) {
 ...
 */
export function eachTree(treeData: any[], fieldNames: FieldNames, callBack: Fn, parentNode = {}) {
