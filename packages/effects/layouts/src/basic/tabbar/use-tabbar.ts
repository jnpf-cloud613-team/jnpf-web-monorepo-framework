...
import { ArrowLeftToLine, ArrowRightLeft, ArrowRightToLine, FoldHorizontal, Fullscreen, Minimize2, Pin, PinOff, RotateCw, Star, X } from '@vben/icons';
...
export function useTabbar() {
  ...
  const { closeAllTabs, closeCurrentTab, closeLeftTabs, closeOtherTabs, closeRightTabs, closeTabByKey, getTabDisableState, refreshTab, toggleTabPin } =
    useTabs();

  const { createCollectMenu } = globalShareState.getApi();
  const { createMessage } = globalShareState.getMessage();
  ...
  // 收藏菜单
  const collectMenu = async (tab) => {
    const modelId = tab.meta?.modelId || '';
    if (!modelId) return;
    createCollectMenu(modelId).then((res) => {
  ...
  function wrapperTabLocale(tab: RouteLocationNormalizedGeneric) {
    return {
      ...
      meta: {
        ...tab?.meta,
        title: $t((tab?.meta?.i18nCode as string) || '', tab?.meta?.title as string),
      },
    };
  }
  ...
  const createContextMenus = (tab: TabDefinition) => {
    ...
    const menus: IContextMenuItem[] = [
      ...
      },
      {
        ...
        separator: !!createCollectMenu && tabbarStore.getMenuList.includes('collect'),
      },
    ];
    const collectItem = {
      disabled: !tab?.meta?.modelId,
      handler: async () => {
        await collectMenu(tab);
      },
      icon: Star,
      key: 'collect',
      text: $t('preferences.tabbar.contextMenu.collect'),
    };
    if (createCollectMenu) menus.push(collectItem);
