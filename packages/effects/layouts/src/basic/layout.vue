...
import { CheckUpdates, Preferences } from '../widgets';
...
// 侧边多列菜单
const { extraActiveMenu, extraMenus, handleDefaultSelect, handleMenuMouseEnter, handleMixedMenuSelect, handleSideMouseLeave, sidebarExtraVisible } =
...
function autoCollapseMenuByRouteMeta(route: RouteLocationNormalizedLoaded) {
  // 只在双列模式下生效
  if (preferences.app.layout === 'sidebar-mixed-nav' && route.meta && route.meta.hideInMenu) {
...
<template>
  <VbenAdminLayout
    ...
    @update:sidebar-collapse="(value: boolean | undefined) => updatePreferences({ sidebar: { collapsed: value } })"
    @update:sidebar-enable="(value: boolean | undefined) => updatePreferences({ sidebar: { enable: value } })"
    @update:sidebar-expand-on-hover="(value: boolean | undefined) => updatePreferences({ sidebar: { expandOnHover: value } })"
    @update:sidebar-extra-collapse="(value: boolean | undefined) => updatePreferences({ sidebar: { extraCollapse: value } })">
    <!-- logo -->
    <template #logo v-if="preferences.logo.enable">
    ...
    <template #extra>
      ...
      <CheckUpdates v-if="preferences.app.enableCheckUpdates && false" :check-updates-interval="preferences.app.checkUpdatesInterval" />
