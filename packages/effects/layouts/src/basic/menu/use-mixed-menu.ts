...
import { findRootMenuByPath, mapTree } from '@vben/utils';
...
function useMixedMenu() {
  ...
  const handleMenuSelect = (key: string, mode?: string) => {
    ...
    if (_splitSideMenus.length === 0) {
      ...
      const progenyList = getProgenyList(_splitSideMenus);
      if (progenyList.length) {
        const firstChild = progenyList[0];
        navigation(firstChild.path);
        return;
      }
      if (rootMenu && preferences.sidebar.autoActivateChild) {
  ...
  function getProgenyList(data: any) {
    const list: any = [];
    mapTree(data, (menu: any) => {
      if (![0, 1, 6].includes(menu.type)) {
        list.push(menu);
      }
      return menu;
    });
