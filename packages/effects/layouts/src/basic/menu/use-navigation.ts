...
function useNavigation() {
  ...
  const navigation = async (path: string, isReplace: boolean = false) => {
    try {
      const route = routeMetaMap.get(path);
      const { openInNewWindow = false, query = {}, type, link } = route?.meta ?? {};
      if (type === 0 || (type === 1 && !route?.redirect)) return;
