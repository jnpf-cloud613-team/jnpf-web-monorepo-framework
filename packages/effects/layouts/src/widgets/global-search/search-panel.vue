...
import { CornerDownLeft, SearchX } from '@vben/icons';
...
const props = withDefaults(defineProps<{ keyword?: string; menus?: ExMenuRecordRaw[] }>(), {
  keyword: '',
  menus: () => [],
});
const emit = defineEmits<{ close: [] }>();

type ExMenuRecordRaw = MenuRecordRaw & {
  treeName?: string;
};

const { navigation } = useNavigation();
const activeIndex = ref(-1);
const searchItems = shallowRef<ExMenuRecordRaw[]>([]);
const searchResults = ref<ExMenuRecordRaw[]>([]);
...
function search(searchKey: string) {
  ...
  traverseTreeValues(searchItems.value, (item) => {
    ...
    if (item.type !== 0 && item.type !== 1 && reg.test((item.fullName || '').toLowerCase())) {
      results.push(item);
    }
  });
  ...
  searchItems.value = mapTree(props.menus, (item: any) => {
    ...
    if (item.children?.length) {
      item.children.forEach((child) => {
        child.parentNodes = [...(item.parentNodes ?? []), omit(item, ['children', 'parent', 'parentNodes', 'parents'])];
      });
    }
    let treeName = '';
    if (item.parentNodes?.length) {
      for (let i = 0; i < item.parentNodes.length; i++) {
        const e = item.parentNodes[i];
        const name = e?.i18nCode ? $t(e.i18nCode, e.name) : e.name;

        treeName += i > 0 ? ` > ${name}` : name;
      }
    }
    const fullName = item?.i18nCode ? $t(item.i18nCode, item.name) : item.name;

    treeName += treeName ? ` > ${fullName}` : fullName;
    return {
      ...item,
      fullName,
      treeName,
    ...
    <div class="!flex h-full justify-center px-2 sm:max-h-[450px]">
      ...
      <ul v-show="searchResults.length > 0" class="w-full">
        <li
          ...
          <span class="flex-1">{{ item.treeName }}</span>
          <div class="flex-center rounded-full p-1">
