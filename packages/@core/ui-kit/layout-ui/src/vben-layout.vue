...
const tabbarStyle = computed((): CSSProperties => {
  ...
  if (!isMixedNav.value || props.sidebarHidden) {
    ...
    width = `calc(100% - 20px - ${sidebarCollapse.value ? getSidebarWidth.value : onHoveringWidth}px)`;
  } else {
...
const contentStyle = computed((): CSSProperties => {
  ...
  return {
    marginTop:
      fixed && !isFullContent.value && !headerIsHidden.value && (!isHeaderAutoMode.value || scrollY.value < headerWrapperHeight.value)
...
<template>
  <div class="global-layout relative flex h-full w-full" :class="`${bgType}-bg`">
    <LayoutSidebar
      ...
      <div
        ...
        <LayoutHeader
          ...
          <template #toggle-button>
            <VbenIconButton v-if="showHeaderToggleButton" class="global-header-icon my-0 mr-1 rounded-md" @click="handleHeaderToggle">
              <Menu class="size-4" />
            </VbenIconButton>
          </template>
